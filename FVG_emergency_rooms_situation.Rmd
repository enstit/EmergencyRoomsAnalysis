...
```{r}
attendances %>%
  filter(emergency_room == filter(median_loads, median_load>=10)$emergency_room) %>% # We remove Emergency Rooms with less than 10 patients as median, to clear the plot
  filter(waiting_time<as_hms('08:00:00') ) %>% 
...
```
```{r echo=FALSE}
ggsave('images/06_week-priority_bars.jpg', device='jpeg', width=250, height=150, units='mm', dpi=640)
```
...
```
```{r echo=FALSE}
anim_save('images/10_day-priority_bars.gif')
```
